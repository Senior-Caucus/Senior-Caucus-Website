...
export default async function DepartmentSection(props: DepartmentSectionProps) {
    return (
        <div className={`w-screen h-screen md:h-[60vh] grid grid-cols-12 grid-rows-12 ${props.className !== null ? props.className:""}`} >
            ...
            <div className="text-1xl inter-extralight row-start-2 md:row-start-3 md:row-span-8 md:col-start-1 md:col-span-9 col-start-1 col-span-9 row-span-1 ml-3 mt-4 md:mt-3">
                {props.departmentDescription}
            </div>
            <div className="col-start-1 row-start-5 lg:row-start-5 lg:row-span-6 lg:col-start-1 row-span-full col-span-5 lg:col-span-6 grid grid-cols-1 grid-rows-2 md:grid-cols-2 md:grid-rows-1 w-screen h-auto -mt-10 md:-mt-4 -ml-1">
                <PersonBox
